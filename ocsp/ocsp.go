...
func (s StandardSigner) Sign(req SignRequest) ([]byte, error) {
	...
	if bytes.Compare(req.Certificate.RawIssuer, issuer.RawSubject) != 0 {
		return nil, cferr.New(cferr.OCSPError, cferr.IssuerMismatch)
	}

	err := req.Certificate.CheckSignatureFrom(issuer)
	if err != nil {
	...
	if s.dynamic {
		certificate = s.getCaCert()
	}
	if issuer == s.responder || bytes.Equal(issuer.Raw, s.responder.Raw) {
		certificate = nil
	}
	...
	if s.dynamic {
		return ocsp.CreateResponse(s.getCaCert(), s.getCaCert(), template, s.getPrivKey())
	}
