...
type Signer struct {
	...
	// Dynamic
	dynamic    bool
	getPriv    func() crypto.Signer
	getCaCert  func() *x509.Certificate
	getSigAlgo func() x509.SignatureAlgorithm
}

// NewDynamicSigner 动态 Signer, 从 func 获取私钥和证书
func NewDynamicSigner(getPriv func() crypto.Signer, getCaCert func() *x509.Certificate, getSigAlgo func() x509.SignatureAlgorithm, policy *config.Signing) (*Signer, error) {
	if policy == nil {
		policy = &config.Signing{
...
func (s *Signer) sign(template *x509.Certificate, lintErrLevel lint.LintStatus, lintRegistry lint.Registry) (cert []byte, err error) {
	...
	var derBytes []byte
	if s.dynamic {
		derBytes, err = x509.CreateCertificate(rand.Reader, template, s.getCaCert(), template.PublicKey, s.getPriv())
	} else {
...
func (s *Signer) Sign(req signer.SignRequest) (cert []byte, err error) {
	...
	if s.dbAccessor != nil {
		...
		var certRecord = certdb.CertificateRecord{
			...
			// CA Label 储存 profile 名字, 用于区分 role
			// CALabel:    req.Label,
			CALabel:    req.Profile,
			Status:     "good",
		...
		sans := make([]string, 0, len(certTBS.URIs)+len(certTBS.DNSNames)+len(certTBS.IPAddresses))
		for _, uri := range certTBS.URIs {
			sans = append(sans, uri.String())
		}
		if len(certTBS.DNSNames) > 0 {
			sans = append(sans, certTBS.DNSNames...)
		}
		for _, ip := range certTBS.IPAddresses {
			sans = append(sans, ip.String())
		}
		if err := certRecord.SetSANs(sans); err != nil {
			return nil, err
		}
