...
func (srv *server) post(url string, jsonData []byte) (*api.Response, error) {
	...
	if resp.StatusCode != http.StatusOK {
		log.Errorf("http error with %s, body: %s", url, string(body))
...
func (srv *server) Info(jsonData []byte) (*info.Resp, error) {
	...
	if val, ok := res["trust_certificates"]; ok {
		if trustCerts, ok := val.([]interface{}); ok {
			for _, trustCert := range trustCerts {
				info.TrustCertificates = append(info.TrustCertificates, trustCert.(string))
			}
		}
	}
